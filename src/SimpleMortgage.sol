 ...
 */
contract SimpleMortgage is ERC721, Ownable, ReentrancyGuard, Pausable {
     ...
     */
    function purchaseProperty(uint256 propertyPrice) external nonReentrant whenNotPaused returns (uint256) {
        ...
        require(usdt.transferFrom(msg.sender, address(this), downPayment), "Down payment failed");
        require(usdt.transferFrom(msg.sender, treasury, platformFee), "Platform fee failed");
        ...
        emit MortgageCreated(tokenId, msg.sender, propertyPrice, loanAmount, monthlyPayment);
    ...
    function makePayment(uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        // Transfer payment
        require(usdt.transferFrom(msg.sender, address(this), m.monthlyPayment), "Payment failed");
        
        // Calculate interest vs principal
        uint256 remainingBalance = m.loanAmount - ((m.loanAmount * m.paymentsMade) / TERM_MONTHS);
        uint256 interestPortion = (remainingBalance * INTEREST_RATE_BPS) / (10000 * 12); // Monthly interest
        ...
        if (stakingPool != address(0) && interestPortion > 0) {
            require(usdt.transfer(stakingPool, interestPortion), "Interest transfer failed");
        ...
        emit PaymentMade(tokenId, m.paymentsMade, interestPortion, principalPortion);
     ...
     */
    function appraiseProperty(uint256 tokenId, uint256 appraisedValue) external onlyOwner {
        Mortgage storage m = mortgages[tokenId];
     ...
     */
    function distributeAppreciation(uint256 tokenId) external onlyOwner nonReentrant {
        Mortgage storage m = mortgages[tokenId];
        ...
        require(usdt.transfer(treasury, a.treasuryShare), "Treasury transfer failed");
        
        if (stakingPool != address(0)) {
            require(usdt.transfer(stakingPool, a.stakerShare), "Staker transfer failed");
